const fetchFacebookData = async (accessToken, pageId, isPageToken) => {
  try {
    const baseEndpoint = isPageToken ? `https://graph.facebook.com/v13.0/${pageId}` : 'https://graph.facebook.com/v13.0/me';

    const response = await fetch(`${baseEndpoint}/posts?fields=message,attachments{caption,media,type,subattachments{media}},stream_status&access_token=${accessToken}`);
